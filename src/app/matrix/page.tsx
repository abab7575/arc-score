...
export default function PublicMatrixPage() {
  ...
  useEffect(() => {
    fetch("/api/matrix")
      .then((r) => {
        if (!r.ok) throw new Error("Matrix data not available.");
        return r.json();
      })
      .then((d) => {
        if (d.error) throw new Error(d.error);
        const scanned: BrandRow[] = d.brands
          .filter((b: Record<string, unknown>) => b.scanned)
          .map((b: Record<string, unknown>) => ({
            ...
            scannedAt: (b.scannedAt as string) ?? null,
          }));
        setBrands(scanned);
        setStats(d.stats);
        setLoading(false);
  ...
  if (error) {
    return (
      <>
        <Navbar />
        <div style={center}>
          <div style={{ ...mono, fontSize: 14, color: COLORS.coral }}>
            Matrix data not available
          </div>
          <div style={{ ...mono, fontSize: 12, color: COLORS.muted, maxWidth: 400, textAlign: "center", marginTop: 8 }}>
            The matrix is being updated. Please check back shortly.
          </div>
        </div>
      </>
    );
  ...
  if (loading || !stats) {
    return (
      <>
        <Navbar />
        <div style={center}>
          <div style={{ ...mono, fontSize: 14 }}>Loading matrix...</div>
        </div>
      </>
    );
  }
