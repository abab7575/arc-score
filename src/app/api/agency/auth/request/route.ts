...
export async function POST(request: NextRequest) {
  ...
  if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) {
    return NextResponse.redirect(new URL("/agency/login?error=Enter+a+valid+business+email", request.url), 303);
  }
  ...
  if (freeEmailDomains.has(emailDomain)) {
    return NextResponse.redirect(new URL("/agency/login?error=Use+your+agency+work+email", request.url), 303);
  }
  const ip = request.headers.get("x-forwarded-for") || "unknown";
  if (!rateLimit(`agency-login:${ip}:${email}`, 3, 60 * 60 * 1000).success) {
    return NextResponse.redirect(new URL("/agency/login?error=Too+many+requests.+Try+again+later.", request.url), 303);
  }
  ...
  if (emailDomain !== workspace.domain) {
    return NextResponse.redirect(new URL("/agency/login?error=Use+an+email+at+the+agency+domain", request.url), 303);
  }
  ...
  try {
    ...
    return NextResponse.redirect(new URL("/agency/login?error=Email+delivery+failed.+Please+try+again.", request.url), 303);
  }
  recordAgencyEvent(workspace.id, "magic_link_requested", { email });
  return NextResponse.redirect(new URL("/agency/login?sent=1", request.url), 303);
}
