...
export async function POST(request: Request) {
  ...
  if (!domain.includes(".")) return NextResponse.redirect(new URL("/agency/sites?error=Enter+a+valid+domain", request.url), 303);
  if (getWorkspaceSites(auth.workspace.id).length >= AGENCY_SITE_LIMIT) {
    return NextResponse.redirect(new URL("/agency/sites?error=Your+50-site+limit+has+been+reached", request.url), 303);
  }
  ...
  if (duplicate) return NextResponse.redirect(new URL("/agency/sites?error=That+site+is+already+in+the+portfolio", request.url), 303);
  try {
    ...
    return NextResponse.redirect(new URL(`/agency/sites?error=${message}`, request.url), 303);
  }
}
