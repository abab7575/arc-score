...
function categoryColor(category: string) {
  const colors: Record<string, string> = {
    ...
    startups: "bg-emerald-500/20 text-emerald-400",
    ai: "bg-purple-500/20 text-purple-400",
    payments: "bg-pink-500/20 text-pink-400",
    analysis: "bg-yellow-500/20 text-yellow-400",
    "ai-agents": "bg-violet-500/20 text-violet-400",
  };
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {TWITTER_ACCOUNTS.map((account) => (
            <a
              ...
              className="flex items-center justify-between px-4 py-3 bg-white/5 border border-white/10 rounded-lg hover:border-sky-500/30 transition-colors group"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-white/30">@{account.handle}</div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <ExternalLink className="w-3.5 h-3.5 text-white/20 group-hover:text-sky-400 transition-colors" />
              </div>
            </a>
          ))}
        </div>
      ...
      <div>
        ...
        <form onSubmit={handleQuickAdd} className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 rounded-lg bg-white/5 border border-white/10 text-white text-sm placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-sky-500"
          />
          <button
            type="submit"
        ...
        {addResult && (
          <p className="text-xs text-white/40 mt-2">{addResult}</p>
        )}
      </div>
    </div>
  );
